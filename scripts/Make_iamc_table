...
def select_time_period(series) :
  ...
  if 'Winter'in current_sheet  :
   a=pd.Series(series[winter_i:winter_e])
   b=pd.Series(series[winter_ii:winter_ee])
   return pd.concat([a,b])
  elif 'summer' in current_sheet or 'Summer' in current_sheet :        
...
def select_metric(series) :
  ...
  if 'peak' in current_sheet :
   return series.max()
  elif 'Percentile' in current_sheet and '50' in current_sheet:
   return series.quantile(0.5)
  elif 'Percentile' in current_sheet and '25' in current_sheet :
...
def make_iamc_table (years ,scenario, simpl, cluster) :
     ...
     file = openpyxl.load_workbook(snakemake.input.iamc_format)   #original IAMC file, official template
     ds_eu, ds_eu_uk, ds_all=([] for i in range(3))  #separate sheets for regions

     for year in years:
        overrides = override_component_attrs(snakemake.input.overrides)
        n = pypsa.Network(networks_dict[(simpl,cluster,lv,scenario,year)], override_component_attrs=overrides)
        industry_demand=pd.read_table('resources/industrial_energy_demand_elec_s{}_{}_{}.csv'.format(simpl, cluster, year) ,delimiter=',',index_col=0) ##.format(year)##
        #h = pd.date_range(freq='h', **snakemake.config['snapshots'])
        h=int(scenario[str.find(scenario,'-')+1 : str.find(scenario, 'H')])           #hourly resolution
        print('hourly res is:', h)

        year_sub=2015 if year==2020 else year    # For the 2015 column of the table, information of 2020 is used.
        ...
        for country in countries:  
            ...
            for sheet in sheets.keys() :
              if year==snakemake.config['scenario']['planning_horizons'][0]:
                ...
                if country == 'EU' :  #Separate worksheet for EU so other values could be added as the program runs
                  ds_eu.append(file[sheet +' '+str(country)]) 
                if country == 'EU_UK' :  #Separate worksheet for EU+UK
                  ds_eu_uk.append(file[sheet +' '+str(country)])   
                if country == 'All' :  #Separate worksheet for All counties being modeled
            ...
            #Fuel : Natural gas(OCGT, CCGT, CHP, CHP CC)       
               # **CHP fuels calculated bsed on share of electricity and heat, 'safe_div' is used in case there is division by zero:                                               
            ...
            for sheet in sheets.keys() :
              ...
              for v in var[sh].keys():
                 ...
                 ds[sh].cell(row=ro, column=2).value = 'Scenario'  ###TODO name of scenario
                 ds[sh].cell(row=ro, column=3).value = country #region
                
        ### EU, EU_UK, and All adjustments 
...
if __name__ == "__main__":
    if 'snakemake' not in globals():
        ...
        snakemake = mock_snakemake('make_iamc_table')

    path= snakemake.config['results_dir'] + snakemake.config['run'] + '/csvs/'

    networks_dict = {
        (simpl, cluster, lv, opt+sector_opt, planning_horizon) :
    ...
    years = [year for year in planning_horizons if year in [2020, 2030, 2050]]   #Should IAM only include these years?

    for simpl in  snakemake.config['scenario']['simpl']:
     for cluster in snakemake.config['scenario']['clusters']:
          for lv in  snakemake.config['scenario']['lv'] :
            for i, scenario in enumerate(snakemake.config['scenario']['sector_opts']) :   #check if I should add opt
                   print(i, scenario, simpl)
                   print(path)   
                   df = make_iamc_table(years ,scenario, simpl, cluster)
                   df.save(path+"iamc_table_s{}_{}_lv{}_{}_{}.csv".format(simpl, cluster,lv, snakemake.config['scenario']['opts'][0], scenario))      #snakemake.output.results_iamc)                                                                
                    
                    
     format= openpyxl.load_workbook(snakemake.input.iamc_format)   
     format.save(snakemake.output.results_iamc)
